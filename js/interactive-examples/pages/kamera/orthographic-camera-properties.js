...
const iExample = new InteractiveExample(5, null);
...
iExample.onInit = () => {
    ...
    gui.add(size, "value").min(0.1).max(5).step(0.01).name("velikost").onChange(() => {
        const aspectRatio = iExample.canvasContainer.clientWidth / 2 / iExample.canvasContainer.clientHeight;

        camera.left = -size.value * aspectRatio;
        camera.right = size.value * aspectRatio;
        camera.top = size.value;
        camera.bottom = -size.value;
...
iExample.onResize = (newWidth, newHeight) => {
    ...
    const aspectRatio = newWidth / 2 / newHeight;
